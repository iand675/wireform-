...
:::note
This is the **living specification**: the draft changelog at the top of this document tracks every revision, most recently the implementation-driven Draft 27. The reference implementation is [`wireform-lattice`](../haskell/); the worked GraphQL comparison corpus is [a companion document](../corpus/).
:::
...
Draft 27 is the reference-implementation draft: every change resolves a question the wireform implementation (the `wireform-lattice` origin and client in Haskell, `lattice-ts` in TypeScript, and the Varnish / Cloudflare Worker cache tiers) had to answer concretely, and none changes the model. **Interfaces are declared** (Section 3.4): earlier drafts used interface names in relationship targets and `fragment ... on` positions without ever declaring them; an `interface` declaration now names the common fields and entities opt in with `implements`, from which the semantic model's membership map derives. A collection targeting an interface or inline union may name a link column that no member declares — a storage-level join column — but a column declared by *some* members must be declared by all. **Roots may be parameter-backed** (Section 3.4): `list search(text: Text) of (Human | Droid | Starship)` declares a list root whose collection groups by its parameters rather than by a target field; `by <field>` remains the field-backed form. **Write sets can name created entities** (Section 11.4): `writes Review(new)` scopes an entity of that type whose key exists only after commit. **Every derived encoding is pinned** so independent implementations agree byte-for-byte: hash truncations and prefixes (Section 5.1), canonical JSON (Section 3.5.3), the cursor layout (Section 3.2), canonical-text rendering separators (Section 5.1), and the refs-projection wire form (Section 6.8, `unchanged` records). **Dictionaries are optional on the inline form** (Section 5.2): `dv` may be omitted, because the dominant browser encoder (`CompressionStream`) has no dictionary support; the dictionary path also gains a consumption note (raw-DEFLATE inflaters install a preset dictionary up front rather than waiting for `Z_NEED_DICT`). Smaller clarifications: reserved URL parameter names are rejected as variable names (Section 4.8 rule 7); `limit` is accepted as a surface synonym for `first` and normalized away in canonical text (Section 4.2, resolving a Draft 24 example inconsistency); plan identity may coarsen pertinent declarations to whole-declaration granularity, which only ever moves more plans than strictly necessary, never fewer (Section 7.3); `effect natural` takes an optional quoted justification in the IDL (Section 3.4); the Section 3.4 example is self-contained (it previously referenced undeclared `Url`/`Markdown` types and an undeclared `rank` ordering column); and parsers accept `String`/`Int`/`Float`/`Boolean` as aliases for `Text`/`I32`/`F64`/`Bool`, with canonical documents emitting the canonical names. A later amendment specifies **asynchronous and out-of-band invalidation** (Section 11.5): why at-least-once, reordered purge delivery is safe (invalidation-not-population, idempotent commuting purges, soft purge under SWR), row-driven key derivation over before/after images so out-of-band writers — batch jobs, stream consumers picking changes off a topic — feed the same pipeline from their own commit point, the purge-at-truth-commit rule that closes the intent-time stale-refill race, and a TLA+ model of these claims shipping with the reference implementation (`wireform-lattice/tla/`). A second amendment adds **co-keyed entities** (Section 3.8): `entity P joins B` (1:1 identifying join — adjacent truth, independent ver and invalidation) and `entity R refines B` (subclass view — same truth, shared ver, family-wide surrogate-key fan-out and tombstone cascade), pinning how ids reused across entity types map to identity, versioning, and invalidation coupling; unrelated bare-id reuse was already a non-event because identity is the type-qualified pair. A third amendment adds **cardinality declarations** (Sections 3.4–3.6): `has one` now promises resolution (`lattice:cardinality` Edge-scoped error on a dangle) with `has one?` as the declared-optional form, bounded collections take a `min N` floor (`lattice:collection-underflow`, mirror of overflow), and the type language gains the nonempty list `[t]+` (with `[t]+?` for omitted-or-meaningful and element optionality `[t?]` rejected) — shared by fields, mutation inputs, and variables, so required-nonempty inputs come from the same declaration.

Draft 26 specifies what Draft 25 asserted. **Origin coalescing** gets its own section (6.9): per-type accumulation windows with a first-entrant deadline (`coalesceWindowMs` in discovery), single-flight per `(type, key)`, the loads-are-policy-free/rendering-is-policy-full argument for why cross-principal batching is safe by construction, response independence on the wire, per-round failure isolation, the version-pinned mismatch rule (`404 lattice:version-unavailable`, `no-store`, `Content-Location` to the unpinned URL, protecting the immutable-URL claim), layering against CDN request collapsing, and a `lattice.coalesce.wait` histogram. **A normative grammar** for the query language (Section 4.8): EBNF over a token stream with GraphQL-style ignored tokens, lexical productions, eight static rules (one query per document; reserved names; aliases and general directives excluded by the grammar itself; `@depth` semantics; edge/scalar selection rules; declared-arguments-only including pagination-argument exclusivity; variable use/declaration; spread resolution and acyclicity), and the closure property that canonical text is itself a sentence of the grammar with fragments expanded.

Draft 25 made pagination opt-in and tightens native-HTTP usage. **Collections are now bounded or paginated** (Section 3.6): a `has many` bounded by its domain (tags, roles, line items) declares `max N`, returns its whole set as a plain ref array, and takes no pagination arguments; only collections that grow without bound (`page N max M`, keyset order) are paginated. Both still declare a cardinality cap so the fan-out budget stays bounded; exceeding a bounded cap is an `Edge`-scoped `lattice:collection-overflow` rather than silent truncation (opt-in `truncate` available). This removes the ceremony of forcing cursors onto small lists while keeping the improvements over GraphQL: small lists are plain arrays instead of Relay `{edges,pageInfo}` envelopes, and paginated lists keep derivable keyset cursors. **Native-HTTP pass**: version-pinned point fetches (`/e/Post/17?ver=e41`) are `Cache-Control: immutable`, making partial loading revalidation-free; `stale-if-error` (RFC 5861) is recommended on shared slices; `429`/`503` MUST carry `Retry-After` (new `lattice:rate-limited`); custom headers are RFC 8941 Structured Field Values; introductions return `Content-Location` for the canonical GET; and the note explains why the refs projection stays a URL parameter rather than `Prefer` (it would force `Vary`).

Draft 24 returned the query language (Section 4) to a GraphQL-shaped surface: brace-nested traversal, fields and edges selected inline, `...Fragment` spreads and `fragment Name on Type { ... }` for reuse, `... on Type { ... }` inline fragments for interfaces, pagination and grouping-key filtering as ordinary field arguments, and `@depth(n)` as the sole sanctioned directive. The motivation is transport: the inline GET form (Section 6.2) carries compressed query text in the URL, and brace-nested selection packs far more per character than the SQL keyword-clause form of Drafts 21 to 23, so more queries fit under the URL budget before falling back to QUERY or POST. The domain-model schema (Section 3.4) is unchanged, since it never travels in a URL; GraphQL itself separates its schema language from its query language the same way. The term "view" becomes "fragment" throughout; schema-declared reusable projections are now schema fragments; `lattice:view-shadow` becomes `lattice:fragment-shadow`. Aliases and `@include`/`@skip` remain absent. Semantics, wire format, plan model, invalidation, and identity are unchanged.

Draft 23 removed the separate `index` declaration and the `indexed by` reference that Draft 22 still carried. A `has many` relationship and a `list` root now define their **collection** inline: `has many comments: Comment by postId ordered by createdAt desc page 20 max 100`. The collection's cache-tag family is auto-named after its path (`Post.comments`, `feed`) and shareable with `as <name>` only when two readers need one tag family; its grouping key (the term *discriminant* is retained but glossed) defaults to the link field. Mutations invalidate collections by path, `feed(Post.orgId)`. Wire cache tags follow suit (`feed:123`, `Post.comments:17`). This eliminates the name-matching indirection between a relationship and a separately-declared index, which was the remaining source of confusion in the schema surface. Semantic model (Section 3.1), query surface, wire format, and invalidation guarantees are otherwise unchanged.

Draft 22 reworked the schema (IDL) surface for readability, with no change to the semantic model (Section 3.1) it is sugar over. The relationship arrow and `via` are gone: a to-one relationship is `has one author: User by authorId` (follow a foreign key), a to-many is `has many comments: Comment ... ordered by ..., page N max M`. Roots become `list name of Type ...` (many) and `get name of Type ...` (one). Field and root visibility reads as sentences, `visible when caller.org = orgId`, `private`, with a per-entity `visible to all by default` / `private by default` baseline so no field's visibility is silent. Entity headers are `entity Post by id { ... }` with a `fetch by id: <policy>` clause for the by-ref (`nodes`) policy; mutations use `returns Type`, `allow when <pred>`, and `Post(post)` write-set scopes. A "reading the schema" key is added to Section 3.4. Query-side view declarations switch to parenthesized parameters for consistency. (Draft 22's `indexed by` reference is itself superseded by Draft 23's inline collections.)

Draft 21 recast the query language (Section 4) in SQL-`SELECT` form: projection list, `FROM`, index-parameter `WHERE`, `LIMIT`, and `AFTER`/`BEFORE`/`AROUND` cursor clauses, with reusable projections as `WITH` CTEs (local, inlined) and `VIEW` declarations (schema, late-bound), the CTE-versus-view distinction mapping onto Lattice's local-versus-published binding semantics exactly. Multiple roots become multiple `;`-separated `SELECT` statements; recursion is a `DEPTH n` clause; interface dispatch is a `(Type: View | ...)` alternation. New Section 4.7 tabulates every SQL capability withheld (arbitrary `JOIN`/`WHERE`/`ORDER BY`/`GROUP BY`, relational-algebra closure, set operations) against the caching or invalidation reason for withholding it, making explicit that Lattice is SQL-flavored on the surface and deliberately not relationally complete. Semantics, wire format, plan model, and identity are unchanged; the term "selection" gives way to "view" throughout, and `lattice:selection-shadow` reverts to `lattice:view-shadow`. Supersedes Draft 20's `select ... on Type` surface.

Draft 20 (superseded) redesigned the query surface away from the overloaded `->` toward named `select` blocks and `:` projection; Draft 21 keeps that de-overloading and takes it into SQL clause form.

Draft 19 adds a third response tier for partial failure: `207 Multi-Status` (Section 9.4.6) alongside `200` and whole-request `4xx`/`5xx`, reusing the status code's generic registration without WebDAV's body format. Query responses use it opportunistically (only when degradation is known before headers commit; the in-stream signal remains authoritative regardless); mutation and batch responses use it whenever the response contains any scoped error, since the write-then-render execution order usually makes the outcome known in time. A `Lattice-Outcome` trailer gives header-only infrastructure a best-effort mirror of the in-body signal. This also surfaced and fixed an inconsistency from Draft 18: `complete` had meant "nothing failed" for query responses but "reached a verdict" for batch responses; it now uniformly means the latter everywhere (Section 9.4.4), with degradation tracked as the independent, separate fact it always was for caching purposes.

Draft 18 rebuilds in-stream error reporting (Section 9.4) around a formal, protocol-defined `Scope` sum (Entity, Field, Edge, Root, Item) that generalizes scoping down to a single field within an otherwise-good entity, with a bare-ref shorthand for the common whole-entity case; splits the error vocabulary into protocol codes (the existing `lattice:` namespace, now shared with whole-request problem types) and declared domain error sums that mutations may opt into, reusing the ordinary ADT and compatibility machinery with no new checker rule; states the elided-versus-error hygiene rule and the exact, mechanical test for whether a mutation's effect committed despite a scoped rendering failure; and generalizes the caching treatment (immediate self-purge on query-side scoped failures, no-op-but-signaling on mutation responses that are already no-store) uniformly across the new scope granularities. Section 15 now distinguishes whole-request problem details from in-stream scoped errors explicitly. `MutationDef` gains `errors`; the batch mutation example from Draft 17 is retrofitted to the new format.

Draft 17 adds two capabilities. **Multiple roots per query** (Section 4.3): queries may declare any number of roots, same or different types, each independently paginated and independently sliced via the existing path join; guidance on when a fixed-layout multi-root load fits versus independent single-root queries for scrollable panels. **Batch mutations** (Section 11.8): a declared `batch` policy admits array invocation of one mutation over many inputs, with envelope-plus-item idempotency keying, an `AllOrNothing`/`BestEffort` atomicity choice constrained by effect class, verb-appropriate collection bindings (PATCH/POST array bodies, DELETE via repeated query parameters, PUT excluded), and per-item scoped results that are terminal outcomes rather than stream damage. Also fixed: a stale IDL syntax seam (`idempotency natural` predated the Draft 7 effect-class model and was never updated).

Draft 16 renames the protocol from "Lattice" (originally "Ledger") throughout: problem-type URIs, media types, header names, OTel attribute prefix, CLI, and discovery path. No normative behavior changes; this is a naming pass. The name draws from the visibility-level join-semilattice of Section 8.1.

Draft 15 adds observability (Section 19): W3C context propagation with outbox-carried span context making invalidation causally traceable, bounded span topology derived from the plan algebra with explain-emitted skeletons, `lattice.*` semantic conventions, named metric instruments, the cached-response telemetry rule, and sampling guidance for linked async work.

Draft 14 is a consolidation release. New in this draft: derived fields with declared read sets, witness validators, information-flow checking, and maintained materialization (Section 3.7); audience facets and relationship-backed claim minting extending the visibility model to group-ACL and follower-scoped systems, with the membership/body split pattern (Section 8.3); snapshot domains generalizing tokens to sharded backends and unifying them with the federation vector (Section 13.1); schema modules making the fusion algebra topology-independent, with a worked cross-module extension example (Section 18); seam repairs from the incremental drafting (plan-slice TTL, refs-projection slicing, undeclared example types).

Prior drafts, summarized: D1-D2 core model (content-addressed queries, entity streams, slicing, surrogate keys, batched loaders); D3-D5 transport ladder, discovery, evolution/compatibility, federation; D6 soundness review (path-join partition, payload/proof split, weak etags, write sets, scoped errors); D7 plan identity replacing epochs, effect-class idempotency with Operations; D8-D9 field masks and the resource profile; D10-D11 type system, collections, enums, pagination; D12-D13 build-time fragments and mutation verb bindings with conditional requests. Full history in version control.
...
5. **Cache dependency tracking is a protocol obligation.** Every response enumerates its surrogate keys; every mutation declares its write set and invalidation footprint; both sides derive their keys from the same schema-declared collection grouping keys, so they agree by construction.
...
where `v1...vN` are the item's keyset column values in canonical wire form (Section 3.5.3) and `canonicalJson` is the canonical JSON defined in Section 5.1. A presented cursor whose embedded `specHash` does not match the collection's current `CursorSpec` is the `410 lattice:cursor-retired` case; a cursor that fails to decode at all is a `400`.

### 3.3 Collections and cache grouping

A `has many` relationship and a `list` root each define a **collection**: a paginated set of related entities, defined inline where the relationship or root is declared. There is no separate index declaration to write and wire up by name; a collection states, in place, three things:
...
Every collection also has a **grouping key** (the formal term is *discriminant*): the value that its cache tags are parameterized by. In a cache tag like `Post.comments:17`, the `17` is the grouping key, the part that says *which* Post's comments this tag refers to. The grouping key defaults to the link field, which is almost always what you want (comments group by their post), so you rarely state it; `grouped by <field>` overrides it when you want coarser invalidation than the link.
...
Guidance: grouping keys at tenant or container granularity (org, thread, folder) keep tag cardinality low and over-invalidation bounded to a tenant's own lists. Finer grouping is legal but interacts with the per-response key budget (Section 10.5).

### 3.4 The IDL

The published schema is a text document. It is meant to read as a domain model first and a set of access-and-cache declarations second, so relationships say `has one` / `has many`, roots say `get` / `list`, and visibility rules read as sentences (`visible when caller.org = orgId`). A `has many` or `list` defines its collection inline (Section 3.3); there are no separate index declarations to cross-reference.
...
- `has one author: User by authorId` is a to-one relationship: the author is the `User` whose key is this Post's `authorId` field. A plain key lookup, nothing more to declare. `has one` means **exactly one**: it is a contract that the edge resolves, and a dangling or missing target is reported as an `Edge`-scoped `lattice:cardinality` error (Section 9.4) rather than silent absence. Where absence is legal — an optional companion, a not-yet-set reference — declare `has one?`, the edge-position sibling of `T?`, and an absent target renders as an ordinary absent to-one. The two must agree with the column: a required `has one` over an optional link field (`editorId: UserId?`) is rejected at elaboration, since an optional column cannot promise a required edge. Codegen maps `has one?` to the target's option type.
- `has many X: T by <field>` is a to-many relationship, a **collection** (Section 3.3): the targets are the `T`s whose `<field>` points back to this entity. A collection is **bounded** (`max N`, whole set returned, no paging, for lists small by nature like tags; optionally `min N` for a floor — `min 1 max 50` requires nonemptiness) or **paginated** (`ordered by <field> <dir>` plus `page N max M`, keyset cursor, for lists that grow, like comments); see Section 3.6. Its cache-grouping key defaults to the link field and is overridden only with `grouped by <field>`; there is no separate index, and the tag family is auto-named (`Post.comments`) or set with `as <name>`. Direction convention: for `has one` the linking field is on this entity and points out; for `has many` it is on the child and points back.
- `interface Character { name: Text }` declares an interface: its common fields, which every implementing entity must declare compatibly. Entities opt in with `entity Human by id implements Character { ... }`; the semantic model's membership map (Section 3.1) derives from the `implements` clauses. Interface names are usable as relationship and root targets and in `fragment ... on` positions; an inline union `(A | B)` is an anonymous interface with no common fields. A collection targeting an interface or union may name a link column no member declares (a storage-level join column, resolved by the loader); a link column declared by some members must be declared by all.
...
- `list name of Type ...` and `get name of Type ...` are the entry points (roots): `list` for a paginated many, `get` for a single entity, each carrying its own visibility. Both may declare parameters, `get hero(episode: Episode?) of (Human | Droid)`. A `list` root is **field-backed** (`by <field>`: the link and grouping key are a target field) or **parameter-backed** (`list search(text: Text) of (...)`: no `by`, and the collection's grouping key is its parameters), so full-text-ish entry points whose input is not a stored column still get a well-defined cache-tag family (`search:{text}`).
- `mutation m(args) returns Type { allow when <pred>; writes <scopes>; invalidates ...; effect <class> }` is Section 11. `Post(post)` in a write set is "the Post whose key is the `post` argument"; `Review(new)` is "a Review this effect creates" (Section 11.4); `feed(Post.orgId)` is "the `feed` collection grouped at the written Post's `orgId`," which invalidates exactly the cached feed pages that could contain the post. `effect natural` takes an optional quoted justification, `effect natural "set-to-state"`, the written rationale Section 11.2 requires.

The formal meaning of every line is the semantic model of Section 3.1; this surface exists so the model can be read without it.

Field arguments (`avatarUrl(size:)`) are declared with types and defaults. Entry points (`list`, `get`) are the only top-level query targets; there is no anonymous root. `invalidates writes` is the common case (footprint equals write set); a mutation may invalidate a superset, never less (Section 11.4). The IDL's canonical text (sorted declarations, normalized whitespace) is what `/schema/{schemaHash}` serves (Section 7.1) and what client code generation consumes; there is no runtime introspection query language, because a static immutable document needs none.
...
- **Type constructors** are first-order (types parameterized by types, no higher kinds) and MUST be fully saturated at every use site; codegen uses native generics where the target has them and monomorphizes where it does not. Builtins: `T?` (option), `[t]` in field-type position (variable-length value list; in edge position `[T]` remains relationship multiplicity, a disjoint grammar position), `[t]+` (**nonempty** list: same wire form, and an empty array is invalid wherever the type governs — mutation inputs and variables are rejected at the request, row data violating it is a `Field`-scoped `lattice:integrity` error; codegen maps it to the target's nonempty-list type where one exists), `Vec n t` (fixed length, checked like `Bytes n`), `Set t`, and `Map k v` for any key type `k`. Composition: `[t]?` (absent, or any list) and `[t]+?` (absent, or nonempty — the spelling for "omitted or meaningful", making a *provided-but-empty* list unrepresentable) are both legal; element optionality `[t?]` is rejected — a list contains values, and an element's absence is its absence from the list. The nonempty marker is defined for lists; other containers may adopt it in a later draft.
...
A type used in both positions gets the meet of the rules. Refinement changes are surfaced rather than classified: refinements are not structural, so whether readers relied on one is not mechanically decidable, and pretending otherwise would launder judgment as checking. This is ordinary variance, applied where most schema checkers approximate: the schema knows every position a type occupies, so the checker need not guess.

### 3.6 Collections: bounded and paginated

Not every `has many` needs pagination. A post's tags, a user's roles, an order's line items are bounded by their nature, and forcing keyset cursors onto them is ceremony that buys nothing. The distinction the schema author makes is between a collection **bounded by its domain** (small, finite, returned whole) and one that **grows without limit** (feeds, threads, logs), and only the second is paginated. Both still declare a maximum cardinality, because the planner's fan-out budget (Section 14.1) needs an upper bound either way; the difference is whether the client pages through that bound or receives it whole.
...
If a post's actual tag count exceeds the cap, the collection is reported with an `Edge`-scoped error (`lattice:collection-overflow`, Section 9.4) rather than silently truncated, which tells the schema author the collection was misdeclared as bounded and should be paginated. An author who wants truncation instead declares `max 50 truncate`, accepting that clients see a partial set. `max` defaults to the origin's `maxPage` (Section 14.1) when omitted, so a plain `has many tags: Tag by postId` is bounded at the origin default.

Bounded collections may also declare a **floor**: `has many lineItems: LineItem by orderId min 1 max 200` requires at least one item, the domain rule "an order has line items" stated where the collection is declared. A scan producing fewer than `min` items emits what exists plus an `Edge`-scoped `lattice:collection-underflow` error (the mirror of overflow: report the integrity violation, keep the rest of the response, degrade to `207` per Section 9.4.6), and codegen maps a `min 1` collection to the target's nonempty-list type. `min` composes with either overflow policy, defaults to `0`, must not exceed `max`, and is rejected on paginated collections — an empty page there is indistinguishable from end-of-pagination, and emptiness checking on an unbounded collection would require the count query the protocol declines to force. Roots take no cardinality declarations at all: a `get` can miss and a `list` can be empty by the nature of lookups; cardinality is a *relationship* contract, where the schema author owns the integrity of the link.
...
**Materialization.** `OnRead` computes in the plan and buys freshness with read cost. `Maintained` stores the value: the outbox relay, on publishing keys that intersect a derivation's read set, enqueues recomputation, which writes the owning entity through a bracketed transaction (write scope: that entity), producing an ordinary `ver` bump and ordinary invalidation. Maintained derivations are eventually consistent, with lag observable through snapshot tokens, and are the specification's answer to denormalization: declared, checked, and fed by the same outbox as everything else.

### 3.8 Co-keyed entities: `joins` and `refines`

Production storage reuses ids across entity types in two ways that mean opposite things, and the schema declares which is meant, because the difference is network-relevant: it decides whose caches a write invalidates.
...
A fragment is a named selection on a type, `fragment Name on Type { ... }`, spread with `...Name`. Fragments are the sole reuse mechanism, and they come in two binding times that correspond to GraphQL's own local-fragment idea and a schema-declared extension of it:

- **Local fragments** are defined in a query document (or a shared `.lq` file and `import`ed), and are **expanded into the canonical text at build time**. The server never sees a fragment as a concept; two builds that expand to the same selection share one identity. This is the ordinary fragment, and the default answer to "share this selection."
...
- **Schema fragments** are declared in the IDL (Section 3.4) and referenced by name. They are **late-bound**: the canonical text keeps the reference, the definition joins the query's pertinent declarations (Section 7.3), and editing one moves the plan id of every referencing query (propagating via plan supersession under compatibility checking). Schema fragments double as the entity-endpoint projection unit (Section 6.7) and the code-generation unit (Section 20.3). Referencing one couples client selections to schema deploys, so it is opt-in; local fragments are the default.

**Parameters**: a fragment may take arguments, `fragment UserByline($size: Int = 48) on User { avatarUrl(size: $size) }`, spread as `...UserByline(size: 96)`; canonicalization erases arguments equal to defaults. **Composition**: a fragment may spread others (`...UserByline` inside `UserProfile`); spread graphs must be acyclic and flatten before expansion, and slicing operates on the expanded selection. A local fragment whose name collides with a schema fragment of the same type is rejected `lattice:fragment-shadow`.

### 4.6 What Lattice withholds, and why

Lattice looks like GraphQL and borrows SQL's discipline about what a cacheable read may do. Every restriction below does work for the caching, authorization, or invalidation model, and a developer coming from either GraphQL or SQL will reach for each; each is withheld on purpose.
...
| `where` on arbitrary fields (SQL) | filter only on a collection's grouping key, as an edge argument | A predicate over an ungrouped field yields membership no surrogate key covers, so no mutation could know to purge it. Grouping-key filters are exactly the ones a cache tag tracks. |
...
The through-line: a Lattice query may only ask for things the schema has pre-committed to serving and invalidating. That is a narrower contract than GraphQL's resolver-can-do-anything or SQL's relational completeness, and it is what lets every response carry a correct, mechanically-derived set of cache tags.

### 4.7 Conditional inclusion, deliberately absent

There is no `@include`/`@skip`. Conditional selection multiplies a query's cache identities by the toggle space, exactly the property content addressing depends on staying small. The replacements, in order of preference: (1) **separate queries**, a second small query issued when a UI state is entered, each caching at its own rate; (2) **shared fragments**, an `import`ed fragment across variants, so n variants are n canonical texts each cache-addressed in its own right; (3) **over-fetch a cheap field** when a variant differs by one inexpensive scalar. Runtime string assembly of per-request variants is discouraged: it recreates the one-shot workload and forfeits tenure.

### 4.8 Grammar

The grammar is normative. It is given in EBNF over a token stream: `[x]` is optional, `{x}` is zero or more, `|` is alternation, quoted strings are case-sensitive terminals. **Ignored tokens** (spaces, tabs, line terminators, commas, and `#`-to-end-of-line comments) may appear between any two tokens and are required only where two adjacent tokens would otherwise lex as one (two Names, or a Name and a NumberValue); commas are never syntax. Everything GraphQL-familiar is deliberate; the differences are the absence of productions (no alias, no general directive, no operation types besides `query`) rather than the presence of new ones.
...
**Static rules.** The grammar is context-free; the following are checked after parsing and before compilation, each rejected as `400 lattice:compile-rejected` with the diagnostic named:

1. A document contains **exactly one** `QueryDef`, any number of `FragmentDef`s and `Import`s.
2. The Names `query`, `fragment`, `import`, `on`, `true`, and `false` are reserved: not usable as fragment names, variable names, or enum values. (This is what disambiguates `... on T {}` from `...fragName` and `EnumValue` from `BooleanValue`.)
...
4. `@depth` is the entire directive grammar. A `Field` carrying `Depth` MUST NOT carry a `SelectionSet`, MUST be an edge whose target type is the type of the enclosing selection, and means: repeat the enclosing selection set at this edge, unrolled `n` levels, the innermost level omitting the recursive edge (Section 4.2). Any other `@` token is a parse error, not a validation error.
...
6. Every argument must be declared: field arguments by the field's IDL declaration; `first`/`after`/`last`/`before`/`around` only on paginated collections, with `first`/`last` mutually exclusive, `after` only with `first`, `before` only with `last`, and `around` exclusive of both; grouping-key arguments on the roots and edges whose collections declare them. Bounded collections take no arguments (Section 3.6). Explicit `null` does not exist as a `Value`; omission is the only spelling of absence.
7. Every declared variable is used and every used variable is declared, with compatible types. An unused declaration is rejected rather than ignored, because it would otherwise vary the canonical text without varying meaning. Variable names additionally must not collide with the reserved URL parameter names (`p`, `slice`, `vc`, `project`, `live`, `d`, `dv`, `intent`), since variables bind as URL query parameters in the hash form (Section 6.1).
...
**Closure under canonicalization.** The canonical text (Section 5.1) is itself a sentence of this grammar, in a restricted form: exactly one `QueryDef`, anonymous (5.1 erases the name, which is why `Name` is optional above), no `Import`s, no local `FragmentDef`s (all expanded), and `FragmentSpread`s only to schema fragments (the late-bound references that survive expansion). A parser for this grammar therefore reads both what developers write and what origins hash, and the difference between the two is one total function, not a second language.
...
1. Applying default values for omitted arguments, then erasing the argument when equal to its default (so stating a default explicitly and omitting it are one identity).
...
The serialization of step 4 is pinned (Draft 27): selections within a set are separated by a single space (`{title author{name}}`); arguments and variable declarations are comma-separated with no spaces (`comments(after:$c,first:3)`, `query($a:Cursor,$b:I32=20){...}`); string values render as RFC 8259 JSON strings; numbers render as canonical JSON numbers (below); inline fragments render `... on Type{...}` and spreads `...Name` / `...Name(size:96)`; `@depth(n)` follows the argument list directly. Wire field keys (`avatarUrl(size:48)`, Section 9.1) use the same argument rendering, so exactly one rendering exists protocol-wide.
...
For URL embedding, canonical text is compressed with raw DEFLATE at a fixed, specified parameterization, using a **schema-derived shared dictionary**: a deterministic function of the schema (field names, type names, common syntax). Dictionaries are content-addressed and immutable:

```
GET /schema/dict/{dictHash}     Cache-Control: public, max-age=31536000, immutable
```

Inline-form URLs (Section 6.2) name their dictionary with a `dv={dictHash}` parameter; `dv` MAY be omitted, meaning the text was compressed with no dictionary — required for encoders without preset-dictionary support (the browser `CompressionStream("deflate-raw")` among them), and always a conforming choice since the origin re-canonicalizes regardless. Origins MUST retain every dictionary they have ever published (each is a few kilobytes; retention is unbounded by design), so a URL minted under any past schema version remains decodable forever and no rollover window exists. Because query text consists mostly of schema identifiers, dictionary compression keeps realistic queries to a few hundred URL bytes.
...
Consumption note (Draft 27): with raw DEFLATE there is no `Z_NEED_DICT` signal, so inflaters install the preset dictionary immediately after initialization (`inflateSetDictionary` before the first `inflate` call, in zlib terms). Bindings that only install dictionaries reactively can equivalently prime the window by prepending the dictionary as non-final stored blocks and discarding that prefix from the output; the wire bytes are ordinary raw DEFLATE either way.
...
Suitable for canonical texts that compress under ~6 KB of URL budget (RFC 9110 recommends intermediaries accept at least 8000 octets of request line). Requires no origin state and no extra round trip. Responses to inline-form requests SHOULD include `Location` with the hash-form URL so clients can upgrade.
...
Browser clients note: QUERY is not CORS-safelisted and incurs a preflight. Another reason it is the introduction verb, not the steady state.
...
`slice=plan` returns the current plan document for the query: its plan id, the slice structure, per-slice claim dependencies, and root-to-slice assignment, derivable from (canonical text, pertinent schema declarations) alone. It contains no entity data and no membership, so it is publicly cacheable regardless of the query's policies with a short TTL; it is the one per-query "current" pointer, playing the role `/schema/current` plays for the whole schema. The immutable form `GET /q/{hash}/plan/{planId}` serves any plan document the instance can still derive, cacheable forever. A request with **no** `slice` parameter is equivalent to `slice=plan`; data slices always name their plan explicitly via `p`.
...
This is the point-fetch counterpart of the hash-form query URL, and it is what makes partial loading cheap: once the digest (Section 10.4) or a refs projection (Section 6.8) has told a client the `(id, ver)` pairs it lacks, it fetches each pinned version and every one is infinitely and shared-cacheably storable, no revalidation, no `If-None-Match` round trip, because a version that exists can never become stale. An unpinned `GET /e/Post/17` returns current data under the ordinary validator-and-TTL rules; the two forms differ only in whether `ver` is present, and the immutable directive is emitted only when it is.
...
and inherits everything from that definition with no separate machinery: content-addressed identity, tenure, compile budgets (mask size is bounded by the per-fragment field cap), the surrogate key `T:k`, and slicing, where each field's level is `nodesPolicy(T) ⊔ policy(field)` and a mixed-level mask splits into slice fetches exactly as a query does. The common case, an all-public mask, is a single publicly cacheable GET. Earlier drafts restricted point fetches to schema fragments to keep URLs enumerable; that restriction contradicted the protocol's central claim that canonicalization plus tenure make client-chosen selections cache-safe, and is withdrawn.
...
which returns only the manifest with `(id, ver)` pairs for every entity the full response would contain. The refs projection is sliced like any data: membership reached through a gated path is ctx or priv membership (Section 8.1), so `project=refs` never reveals through a cheaper slice what the full response would gate. The client then fetches entities it lacks as parallel point fetches (Section 6.7) over the same HTTP/2 or HTTP/3 connection, with field masks derived from the plan's fragments and client-chosen RFC 9218 priorities. The refs projection carries the same surrogate keys and validators as the full slice, so membership freshness is identical across profiles; because it includes versions, partial loading is subtraction rather than a digest header. The refs projection's wire form is the ordinary stream restricted to identity: the manifest, one `unchanged` record (`{"kind":"unchanged","id":...,"ver":...}`, Section 10.4) per entity the full response would contain, and the `end` record, under the same etag, surrogate keys, and snapshot header as the full slice (Draft 27).
...
- **Overhead.** With HPACK/QPACK, marginal request cost is tens of bytes; the operative limits are CDN request pricing and `SETTINGS_MAX_CONCURRENT_STREAMS` (commonly ~100), which bound sensible page sizes. On HTTP/3, per-entity streams recover independently from packet loss; on HTTP/2 over TCP, transport head-of-line blocking erases that advantage.
...
Resource mode moves result assembly to the client, which means the origin sees the same work arrive as a spray of independent point fetches instead of one plan. Coalescing is the mechanism that makes those two arrival patterns execute identically, and it is a conformance requirement, not an optimization, because the alternative is N+1 against storage, reintroduced at the transport layer by clients doing exactly what Section 6.8 tells them to do.

**Mechanics.** The origin maintains, per entity type, an accumulation window. A point fetch that misses origin-locally joins the current window for its type; the window flushes into one set-in map-out loader call (the same loaders queries use, Section 3.1) when either the loader's batch cap is reached or a deadline expires, measured from the window's **first** entrant, not its most recent, so steady arrival cannot extend the window indefinitely. The deadline is single-digit milliseconds by default and published in discovery as `coalesceWindowMs`; implementations MAY flush early when the connection is otherwise idle. Concurrent fetches for the same `(type, key)` are additionally single-flighted: they join one loader slot and each response is rendered from the one loaded row.

**Loads are policy-free; rendering is policy-full.** Coalescing batches fetches from different callers with different claims into one loader call, and this is safe by construction rather than by care: loaders fetch rows by key and know nothing about callers, while visibility is applied at emission, per response, against that response's own slice and claims (Section 8.1). Nothing about the *load* depends on who asked, so no caller can widen another's read by sharing a batch; everything caller-dependent happens after the batch, independently per response. This is the same read-then-render separation the path join already imposes on query execution, applied at the transport seam. Field masks are likewise per-response projection: the loader returns the row (or the union of requested columns, where loaders support column sets), and each response emits only its own mask.
...
**Observability.** `lattice.loader.batch_size` (Section 19.3) is the conformance signal here exactly as it is for query planning: resource-mode traffic whose batch sizes sit at 1 means the window is misconfigured or coalescing is broken. The companion histogram `lattice.coalesce.wait` records time spent in the window per fetch, which is the latency cost being paid for the batching and the number to watch when tuning `coalesceWindowMs`.
...
There is no global schema version on the wire. Instead, every query has a **plan id**: a content hash over its canonical text together with its **pertinent schema declarations**, defined as the transitive closure of declarations its compilation reads: the roots it enters, the entity fields and policies it selects, the schema fragments it references (transitively through spreads), the edge and `nodes` policies it traverses, the collections (link, ordering, grouping, page bounds) of those edges, and the types of the claims those policies reference, all in canonical IDL form. Implementations MAY coarsen pertinent declarations to whole-declaration granularity (the full declaration of every entity, root, fragment, interface, and claim the compilation touched): coarsening moves more plan ids than strictly necessary on a deployment, which is safe; moving fewer is not (Draft 27).
...
- **Claims payload**, in the URL: `vc={base64url(canonicalJson(claims))}`, containing only claims from the schema's registry that this query's ctx slice depends on (the plan says which), canonically serialized so equal claim sets are equal bytes. The payload is part of the cache key by virtue of being part of the URL: no `Vary` support needed, no custom-header preflight for browsers, and correct behavior on caches too limited for `Vary`.
...
Origins MUST verify the proof on every request they serve and MUST reject expired proofs; caches never verify anything. A cache hit is by construction a response some principal with an identical claims payload legitimately received while holding a valid proof.
...
**Multi-audience reads** either fan out per facet (the SDK issues one ctx request per audience and merges in the client store, natural under the resource profile) or run priv. **Feed-like systems** use the membership/body split: the personal membership list (small, unavoidably principal-scoped) travels priv or via a priv refs projection, while entity bodies are fetched per audience through ctx or pub, sharing the heavy bytes at the shared cache. Personalization lives at the membership layer, sharing at the body layer, which is the arrangement production feed caches converge on; here it is expressible in-protocol rather than rebuilt beside it.
...
Reach for a domain error only for a genuine failure to complete the effect. A well-defined alternative outcome that the schema wants to document, "this field does not apply to draft posts", belongs in the field's own type (`T?` or a sum), not in the error channel; `error` means "the answer isn't here," not "here is which documented answer this is." Query-side scoped errors are expected to stay in the protocol vocabulary: a read has no business decision to report, only infrastructure that did or didn't respond in time.
...
For mutations specifically, a scoped error on the output selection never retracts a commit. The effect committed if and only if the response contains at least one `entity`, `tombstone`, or `invalidated` record for that mutation (or, in a batch, for that item); a `Field` or `Edge` error on the *same* entity or item only marks that one piece of the rendered output as unavailable in *this* response. A mutation or item that did not commit produces only an error record, no `entity` or `invalidated` records at all, for that target. This is the entire test a client needs to determine whether a write happened: presence of a non-error record is proof of commit, and nothing else is. A cancel-order call that returns the confirmed cancellation entity plus a `Field`-scoped error on some cosmetic derived field cancelled the order; a cancel-order call that returns only a scoped or unscoped error did not, and the two are never to be confused by a client inspecting only the top-level HTTP status.

#### 9.4.4 `complete`, independent of success

Earlier drafts let `complete:false` mean two different things: "the origin gave up before finishing" and "something in here failed." Those are separate axes and are pulled apart here. `complete` answers only the first question: **did the origin finish attempting the full plan (or the full batch), regardless of what it found?** A loader that times out on one dependency does not usually stop the origin from executing the rest of an unrelated round; it attempts everything, notes one definitive failure, and finishes, which is `complete:true` carrying a scoped `error`. `complete:false` is reserved for genuine truncation, a crash, a timeout that kills the whole execution, a connection drop, before every part of the plan was attempted at all. Whether the response is *degraded* is a wholly separate fact, signaled by the presence of any `error` record, at any scope, checked independently of `complete`.

This is why the batch carve-out in Section 11.8 was never really a carve-out: `complete` there always meant "reached a verdict for every item," which is the general definition above, not an exception to it. Draft 18's query-side text said otherwise; this corrects it.
...
- **Scoped errors on query responses**: the stream completes normally (`complete:true` in the ordinary case; `false` only if truncation happened too), and the origin MUST immediately soft-purge the response's own surrogate keys regardless, so a shared cache serves the degraded response at most briefly under `stale-while-revalidate` before a refresh replaces it. This applies uniformly at every scope granularity: a single missing `Field` on one entity within an otherwise-full page triggers the same immediate self-purge as a whole missing `Entity`, because the alternative, HTTP caching a response with per-field precision, does not exist; HTTP caches operate at response granularity, so the only lever available is how long the whole response lives, not which parts of it.
...
Three tiers, not two: `200` for a fully clean response, a whole-request `4xx`/`5xx` (Section 15) when nothing usable was produced, and **`207 Multi-Status`** for a response that was processed and did produce something, but contains at least one `error` record. `207` reuses the status code's generic IANA registration; it does not imply WebDAV's XML multistatus body, and the response body here is the ordinary NDJSON entity stream exactly as for `200`. `207` is inside the `2xx` range, so generic infrastructure that only distinguishes success from failure by range, load balancer health checks, `fetch()`'s `response.ok`, keeps treating it as a success, while infrastructure that specifically knows to look for it gets a coarse, header-visible signal that the body is worth inspecting. It also happens to fall outside RFC 9111's default-heuristically-cacheable status list, which costs nothing given Section 10.8's stance of never relying on heuristic cacheability, but is a fitting default posture for a response known to be degraded.

Whether a given response can actually *use* `207` depends on whether the origin knows about the degradation before the status line is sent, which streaming makes a hard constraint rather than an implementation detail to paper over:
...
The digest encoding is a Golomb-coded set over hashes of `id@ver` strings with a declared false-positive exponent (`fp=10` ≈ 2^-10), after the design of the HTTP cache-digest work; a false positive means the origin wrongly elides an entity the client lacks, so clients choose `fp` against their tolerance for follow-up point fetches, and origins MUST honor only digests whose declared rate they accept. Kilobyte-scale headers cover tens of thousands of store entries.
...
Neither header may be sent on `pub` or `ctx` slice requests: it would fragment the shared cache key space per client. Both are permitted and effective on `priv` requests and on one-shot POSTs. For shared slices, client-side deduplication against the store achieves the same bandwidth savings one hop later.
...
Co-keyed entities (Section 3.8) shape the entity-key family: a `refines` family shares one truth, so a write to any member mints the entity keys of every member (`User:7` and `AdminUser:7` together), while a `joins` companion mints independently. The fan-out is static from the schema — readers tag only the types they actually touched.

### 10.6 Purging

Invalidation is event-driven from the transactional outbox (Section 11.5) — mutations and out-of-band writes alike. Purges SHOULD be *soft* (mark stale) rather than hard (evict) where the cache supports it: a soft-purged entry pairs with `stale-while-revalidate` to serve stale for one request while a single origin fetch refreshes it, converting invalidation storms into single-flight refreshes.
...
Mutations are named, schema-declared operations invoked as
...
There is no mutation composition language; a business operation is one mutation. Clients wanting multiple *different* effects invoke multiple mutations and get independent idempotency and invalidation semantics for each. Cross-entity atomicity belongs inside one mutation's `Txn`, not in client-side composition. A declared `batch` policy (Section 11.8) is homogeneous repetition of one mutation over many inputs, not composition of different mutations; heterogeneous atomic bundles remain out of scope, for the same reason cross-upstream sagas are out of scope in Section 18.7. A mutation MAY additionally declare `errors`, an ordinary sum type naming its business failure modes, reported via the scoped-error mechanism of Section 9.4.2 rather than as free-form strings.

### 11.2 Idempotency: what is promised, per effect class

Earlier drafts implied a uniform exactly-once story; that claim is only implementable when the entire effect lives inside one ACID transaction, and most consequential mutations do not (they charge cards, send email, call other services, enqueue). The protocol therefore promises exactly one thing uniformly, and more only where the effect class supports it:

**Uniform guarantee: at-most-once acceptance.** For any keyed mutation, the origin accepts `(mutation, principal, key)` at most once within the retention window. The acceptance record is written in a transaction with whatever the initiation transactionally does. Concurrent duplicates of an in-flight key receive `409 lattice:key-in-flight` with `Retry-After`; replays after completion receive the stored response with `Idempotency-Replayed: true`; a replayed key with a mismatched request digest is rejected `422 lattice:key-reuse`. Retention is per-mutation policy (24h default, published in discovery); beyond it a replay re-executes, and clients MUST NOT rely on detection past the window.

**Per class:**

- `Transactional`: the effect and the dedupe record commit together, so acceptance and effect coincide and replay returns the exact stored response. This is end-to-end exactly-once, available precisely because the effect boundary is the transaction boundary.
- `NaturallyIdempotent`: no key required; retries are safe by construction. The compiler requires the written justification because self-certified idempotency is the classic famous-last-words of retry bugs.
- `Workflow`: acceptance transactionally creates an **Operation** entity and enqueues the work (outbox pattern); the response is `202` with `Location: /e/Operation/{id}` and an entity stream containing the Operation in state `pending`. Everything downstream, retries against external systems, compensation, eventual completion, is the workflow engine's responsibility, and the protocol does not pretend otherwise: Lattice guarantees the workflow *starts* at most once per key, and gives its progress a first-class, cacheable, subscribable surface.

**Key material.** Clients SHOULD derive keys from durable business intent (a transfer id, an order id) rather than fresh randomness where such intent exists: a random UUID held only in process memory does not survive the client crash that is precisely the retry scenario the key exists for.

**Replay storage.** Origins choose replay fidelity: the stored response MUST include the manifest and MAY elide entity records, replacing them with refs the client can point-fetch; large mutation outputs thereby do not force large dedupe rows.
...
A `Workflow` mutation's Operation is an ordinary entity (`Operation:{id}`, fields: `status`, `startedAt`, `result` ref, `error`), which means its progress needs no new machinery: poll `GET /e/Operation/{id}` with `If-None-Match`, or subscribe to a live query over it. Completion is a write like any other: the finishing step writes the Operation and the domain entities through write-scoped transactions (Section 11.4) feeding the same outbox, so invalidation, the feed, and read-your-writes via snapshot tokens all apply to workflow completions with no special cases.
...
with `Lattice-Snapshot` carrying the commit's token. The `entity` records carry post-mutation state with fresh `ver`s; the client store applies them like any other stream, which gives **read-your-writes for entity fields with zero follow-up requests**. Deletion effects emit `tombstone` records. The `invalidated` record mirrors the purge set so the client store can locally mark any cached query results whose key sets intersect it as stale.

The returned selection is the mutation's declared output selection, compiled and partitioned like a query; mutation responses are `Cache-Control: no-store` regardless. A scoped error on that output selection never means the effect failed; Section 9.4.3 states the exact test a client uses to tell the two apart, and Section 9.4.6 states when such a response is a `200` versus a `207`.

### 11.4 Write sets: the grounded footprint check

`writes` declares, as a function of the input, the entity and index scopes the effect may modify. It is enforced twice:

- **At runtime**: the `Txn` context brackets every write primitive and rejects, with a server error and rollback, any write outside the declared scopes. The declaration is a bound the transaction cannot exceed, not documentation.
- **At compile time**: the checker verifies `invalidates ⊇ writes` (trivially satisfied by `invalidates writes`, the common spelling), and that every `CollectionScope` names a collection at its declared grouping key, so a footprint at the wrong granularity fails compilation.

Runtime enforcement is what makes the static check meaningful: without it, "under-invalidation is a compile error" would rest on an unverifiable claim about the effect. Over-invalidation remains a performance decision (`WritesPlus`); under-invalidation remains unrepresentable.

A write set may name an entity the effect *creates*: `writes Review(new)` scopes one entity of that type whose key exists only after commit (Draft 27). The runtime bracket checks created rows against it by type; `invalidates writes` then contributes the created entity's key to the purge set exactly as an updated entity's key would be.

For `Workflow` mutations, `writes` declares the closure of scopes the workflow may ever write, including the Operation itself; each workflow step's transaction is bracketed against that closure exactly as a `Transactional` effect is bracketed against its declaration. The initiating request's own transactional writes (Operation row, outbox intent) are always within it by construction.
...
**The pipeline is asynchronous by construction.** Nothing above requires a purge to land before the mutation response is sent, and Section 11.6 exists precisely so that nothing has to: read-your-writes rides the snapshot token, so relay latency degrades only the freshness of *other* principals' shared-cache hits, never correctness. The relay may be an in-process worker, a CDC follower, or a consumer group on a message broker — the outbox rows are already an ordered, resumable log, and Section 18.6's feed cursor is that shape on the wire. Three properties make at-least-once, reordered delivery safe, and they are requirements, not implementation luck: purge is *invalidation* (mark-stale), never cache population, so a duplicated or late purge costs at most one extra revalidation and can never install a wrong body; purging a key is idempotent, and purges of distinct keys commute; and soft purge under `stale-while-revalidate` (Section 10.6) prices the over-purging this tolerance produces at a background refresh rather than a synchronous miss. A channel that *updates* cached bodies instead of invalidating them has none of these tolerances — under reordering it installs stale bodies as fresh — and is therefore not a conforming relay target. `lattice.invalidation.lag` (Section 19.3) is the operational signal for relay health.
...
1. **Key derivation is row-driven, not mutation-driven.** The Section 10.5 keys are a pure function of the schema and the written row images: the entity key `Type:{key}`, plus each affected collection's grouping key evaluated against *both* the before and the after image — a row moving between groups must purge the group it left and the group it joined; creation and deletion have one image each. Where the schema declares co-keyed refinements (Section 3.8), the same derivation includes the family fan-out — a shared-truth row's write mints every family member's entity key, statically. Nothing in the derivation mentions the mutation, so any writer holding the row images can produce byte-identical keys; origin SDKs SHOULD expose the derivation as a standalone function for exactly this consumer.
2. **Purge at truth-commit, never at intent.** In a producer → topic → consumer → serving-store pipeline, the purge is emitted with the *consumer's* transaction, not when the producer publishes. Purging at publish time reintroduces the race the outbox exists to close: the purge lands, a read repopulates the cache with pre-change data at full TTL, the change commits after — and stays invisible until natural expiry, an unbounded staleness window no later message will close. The transactional-outbox placement in this section is not a style preference; it is the atomicity of "truth changed" with "the world will hear about it", and it moves to wherever the commit point moves.

Snapshot domains need no amendment for out-of-band writes: tokens order the serving store itself (Section 13.1), so a consumer's commit advances the domain token exactly as a mutation's does, and guarantee 4 of Section 13.2 is unchanged. `Maintained` derivations (Section 3.7) are this pattern applied internally — an outbox event triggers an asynchronous recompute whose own bracketed commit produces an ordinary `ver` bump and an ordinary outbox row; a topic consumer is the same shape with the trigger outside the origin. The claims of this section — quiescent coherence under at-least-once reordered purge delivery, read-your-writes independent of purge timing, and the staleness race that purging at intent time opens — are model-checked in TLA+; the model ships with the reference implementation (`wireform-lattice/tla/`).
...
The mutation's snapshot token turns "wait until the cache catches up" from a timeout heuristic into a comparison. The protocol offers per-client read-your-writes without imposing global read-after-write on the shared cache, which would require synchronous purge and reintroduce the coupling the design exists to avoid.

### 11.7 Verb bindings and conditional requests

A mutation may bind to an entity-space URL and verb. The binding chooses the wire spelling only; guard, `writes`, `invalidates`, and effect class apply identically, so no business invariant is reachable around by choosing a verb. What the binding adds is HTTP's own semantics, compiler-checked against the effect class:
...
- `If-Match: "{ver}"` on PUT, PATCH, or DELETE succeeds only against that version; mismatch is `412`, and the `412` body is an entity stream carrying current state, so a client rebases and retries with no follow-up fetch.
- `If-None-Match: *` on PUT is create-if-absent; `412` means it already exists.
- Verb-bound mutations SHOULD require a precondition (`428 Precondition Required`) unless the schema author marks the mutation last-writer-wins, so unguarded blind overwrites are a declared decision rather than a default.
...
Mutation responses of bound mutations are the same entity streams as Section 11.3, so read-your-writes, `invalidated` records, and snapshot tokens are unchanged; DELETE responses carry the tombstone.

### 11.8 Batch mutations

A mutation MAY declare a `batch` policy, which admits an array invocation applying the same declared mutation, the same `guard`, `writes`, `effectClass`, and `invalidates`, to many inputs in one request. Nothing about per-item semantics changes; only the invocation and response structure generalize from one item to many.

**Invocation.** For a `Named` binding, the request body's top-level JSON type selects cardinality: an object is the singular call (unchanged), an array is a batch call, each element `{"key": <opaque>?, "input": <Input>}`. This is unambiguous by construction, since every `InputType` is a schema-declared record and a record always serializes as an object (Section 3.5.2), so a bare array can never be a valid singular body.
...
- **DELETE** batch identifies targets via repeated query parameters (`DELETE /e/Notification?id=n1&id=n2&key=op_1&key=op_2`, paired by position), not a request body, since body support on DELETE is inconsistently handled by HTTP clients and intermediaries; this sidesteps that inconsistency entirely.
...
**Atomicity and effect class.** `AllOrNothing` brackets one transaction against the union of every item's declared write set; the compiler admits it only when every batch-eligible mutation's effect class is `Transactional` or `NaturallyIdempotent`, because a `Workflow` mutation's own guarantee, that only initiation is transactional, cannot be strengthened by wrapping N initiations in an outer transaction that promises more than any one of them does. `BestEffort` runs each item in its own bracketed transaction and is therefore the only mode available to `Workflow`-classed mutations, and is available to any class. Downgrading a `Workflow` mutation's batch from (an invalid) `AllOrNothing` is rejected at schema compile time, not at request time.

**Idempotency: envelope and item.** The `Idempotency-Key` header addresses the request as before, whole batch envelope, protecting "did my submission arrive." Each item's own `"key"` field addresses that item, scoped as `(mutation, principal, item key)`, independent of the envelope key. This is the same double-keying production batch order-entry APIs already use: a batch identifier for the submission, and a client order id per leg so that resubmitting an adjusted batch does not reprocess legs that already landed. Items without a key rely solely on envelope-level dedupe, which is adequate for `AllOrNothing` (the whole envelope is one outcome) but means a retried `BestEffort` batch may reprocess previously succeeded items unless their effect class is independently `NaturallyIdempotent`.
...
A per-item `error` record in a batch response is a **terminal outcome for that item**, not stream damage: "already filled" is a legitimate, final answer for `ord_b`, confirmed by 9.4.3's commit test, `ord_b` produced only an error, so it did not cancel; `ord_a` and `ord_c` each produced an `entity` record, so they did. `end.complete` is `true` because the origin reached a verdict for every item (Section 9.4.4); the mixed outcome is what makes the status `207` rather than `200` (Section 9.4.6), and `complete` would only drop to `false` on actual truncation, crash or timeout, before every item was attempted, in which case the client retries the batch and items already given a verdict dedupe away via their item keys. A batch where every item happened to succeed returns this same body form under a plain `200`.
...
**Execution.** Batch effects SHOULD run through the same round-batched machinery as reads where the underlying writes are homogeneous (`UPDATE ... WHERE id IN (...)` for a `BestEffort` batch of field updates, rather than N round-trips to storage): the write-set bracket constrains what may be written, not how many statements accomplish it, so an origin is free to implement a 500-item `BestEffort` batch as one bulk statement per distinguishable write pattern. This is the write-side counterpart of set-in map-out loaders, and keeps a batch mutation's trace bounded (Section 19.2) rather than growing one span per item.

**Budget.** `maxItems` is capped by the schema author and MUST NOT exceed the origin's published `maxBatchItems` (Section 14.1); a request naming more items than the mutation's `maxItems` is rejected `400 lattice:batch-too-large`. A batch invocation of a mutation with no declared `batch` policy is rejected `400 lattice:batch-not-supported`.
...
The origin registers the manifest's surrogate key set against the connection. When the invalidation relay (Section 11.5) publishes an intersecting key, the origin re-executes the plan (single-flighted per query hash + variables + claims payload) and pushes a delta stream: changed `entity` records, `tombstone`s, and a fresh manifest when membership changed. Records are identical to the pull wire format; the client store does not distinguish push from pull.

A subscription's authorization is its proof (Section 8.2), and a long-lived connection MUST NOT outlive it: at proof expiry the origin sends `{"kind":"reauth"}` and terminates the stream after a short grace period unless the client supplies a fresh proof (via the SSE reconnect carrying updated credentials). Without this, a subscription minted under since-revoked claims would keep receiving gated data indefinitely.
...
An origin declares **snapshot domains**: independently ordered keyspaces of its storage (one domain for a single-writer Postgres; one per shard for a sharded backend, with entity types mapped to domains by a declared function of their keys). Every response carries `Lattice-Snapshot` as a vector of `domain=token` pairs for the domains it touched:
...
Tokens are opaque and totally ordered **within a domain** (an LSN, a Spanner timestamp, a commit counter); origins expose the comparison in the SDK, and no order exists across domains. Federation is this mechanism one level up: an upstream is a domain namespace, and the fused vector of Section 18.4 is the same vector with prefixes, so single-writer, sharded, and federated deployments share one consistency vocabulary.

### 13.2 Guarantees

1. **Per-slice, per-domain snapshot isolation.** Within one response, all records from a given domain were computed against that domain's single token in `Lattice-Snapshot`; across domains, no relationship holds, within one origin exactly as across upstreams.
...
`Signed` mode restores build-time allowlisting for closed deployments: the release pipeline signs each query's canonical text, the signature ships in the client artifact and travels in `X-Lattice-Query-Sig`. Trust rides with the client binary; no server-side registry exists in either mode. Confidentiality never depends on admission in either mode; the visibility partition bounds what any query can see. Admission governs resources.
...
This section covers whole-request failures: the HTTP response itself is an error, using RFC 9457 problem details with `lattice:` type URIs, cache directives per Section 10.8. A request that succeeds at the HTTP level but fails to produce some part of its result uses the in-stream `error` record and the `Scope`/vocabulary model of Section 9.4 instead, reported under `200` or `207` per Section 9.4.6; the two mechanisms share the `lattice:` code namespace but apply at different granularities, a whole-request 500 means nothing was returned, a 207 means most of it was, and 200 means all of it was. Notable whole-request types:
...
Mid-stream failures use the `error` record (Section 9.4), since the status line is long gone.
...
- The refs-only projection is a URL parameter (`project=refs`, Section 6.8), not a `Prefer: return=minimal` header, on purpose: `Prefer` would force `Vary: Prefer` and split the shared cache entry, defeating the point. Lattice uses native semantics up to the exact line where they would fragment cache identity, and puts the distinction in the URL past that line, the same reason claims ride in the URL rather than a `Vary`-ed header (Section 8.2).
...
Deployments SHOULD run a **compatibility registry**: a service holding the deployment log (every published schema hash with its deployment timestamp) and each IDL document, the origin's query corpus (canonical texts from the plan memo, exported with their tenure and traffic statistics), and optional client-build attribution gathered from the advisory `Lattice-Client` request header.

The registry is an analysis component, never a serving dependency. The invariant from Section 6.1 stands: an origin serves traffic with no registry available, and total registry loss degrades deploy-time checking, not requests. This is the difference between this design and a Confluent-style registry sitting in the write path: here the registry consumes the memo table, it does not gate it.
...
**Compile compatibility.** Does every query that compiled under the baseline still compile? Additive changes (new entities, fields, indexes, roots, mutations, schema fragments, new optional arguments with defaults, new interface implementors) always preserve it. Removals and type changes can break it; removing a schema fragment, or a field from one, breaks every referencing query and is corpus-checked with the same traffic-weighted report as entity-field removal. Argument default changes are compile-compatible but identity-affecting (Section 5.1); the checker MUST flag them as breaking even though everything compiles.

**Plan stability.** Does any corpus query's plan id change? Any policy change in any of its positions (field, edge, root, `nodes`) can move plan elements between slices under the path join, and any change to a pertinent declaration (index, cursor spec, claim type) changes plan ids even where slicing is unaffected. Because plan identity is per query, the checker reports exactly which queries are disturbed and with what traffic weight, rather than a global yes/no.

**Semantic compatibility.** Does a still-compiling query mean the same thing? Narrowing a policy compiles everywhere but turns visible fields into elisions for some principals; changing an index's ordering changes result order under identical URLs; changing a batch mutation's atomicity from `AllOrNothing` to `BestEffort` (or the reverse) preserves compilation but changes what a partial failure means to a client that has not been rewritten. These are flagged as behavioral breaks distinct from compile breaks, because the blast radius is user-visible wrongness rather than errors. Declaring `batch` on a previously non-batch mutation, and raising `maxItems`, are additive; lowering `maxItems` is breaking for any client sending larger batches, corpus-checked like any other narrowing.

**Cursor compatibility.** Changing a relationship's `CursorSpec` or its index's key columns invalidates outstanding cursors, which live in cached URLs and client state with unbounded lifetime. The embedded spec hash converts misinterpretation into `410 lattice:cursor-retired`; the checker reports spec changes as cursor breaks. Enum declaration order is part of this axis: because enum keyset columns compare in declaration order, inserting or reordering enum constructors is simultaneously a cursor break and a semantic break; **open enums are append-only**, and the checker rejects non-appending changes without an approved override.
...
returning a structured report; nonempty breaks fail the pipeline unless each carries an approved override annotation in the IDL (`@break(approved: "LEDGER-1234")`).
...
`@deprecated(sunset: <date>, note: "...")` on any field, root, or mutation: the element keeps serving, appears with its deprecation metadata in the IDL document (so codegen warns), and the registry tracks its corpus traffic over time. Removal passes the checker only when past sunset or below the traffic threshold, whichever the deployment configures. Deprecation is thereby a measured drain, not a hope.
...
A query selecting `User { name, orders(first: 10) -> Order }` places the `orders` subtree in the priv slice via the ordinary path join, since order history is owner-visible; `name` stays wherever the identity module's policies put it. The commerce module owns `Order`, its `orders` collection and grouping, and the edge policy; identity's ownership of `User` is untouched. In-process, the edge resolves through commerce's loader directly; behind a gateway, through the `nodes` root and subplan machinery of Section 18.3, with no schema difference between the two.
...
- **The outbox carries context.** Each outbox row stores the span context of its committing transaction. Relay work (CDN purges, live-query re-execution, `Maintained` derivation recomputation, feed publication) creates spans that **link** to the originating mutation's span rather than parenting under it, since the work occurs outside the request lifetime. `Workflow` mutations store the initiating context on the Operation entity, and each workflow step links to it. The consequence is complete causal traceability of invalidation: "which write purged this key", "which mutation caused this push", and "what triggered this recomputation" are trace queries, answerable because invalidation flows through one declared pipe and never through side channels.
- **Nothing sensitive rides baggage.** Visibility payloads and proofs MUST NOT be placed in `baggage` or `tracestate`; the claims payload is low-cardinality by design and belongs in span attributes at the origin's discretion, not in propagated context that crosses trust boundaries.
...
This chapter is non-normative but the protocol was designed assuming it exists; several normative choices (path-join slicing, content addressing) trade guessability for inspectability, and these tools are the inspection.

### 20.1 The reference CLI

Canonicalization, hashing, partition, planning, and compatibility diffing are all deterministic functions, and they MUST agree bit-for-bit across every SDK, editor plugin, and origin, or content addressing quietly fragments. The intended mechanism is a single reference implementation shipped as a CLI, plus a test-vector corpus in the specification repository that every independent implementation runs in CI:
...
with the client store typed as a heterogeneous map keyed by (entity type, key), each entry carrying the union of fragments the store has observed and per-fragment projection functions. In TypeScript, each fragment is an exact interface and the store narrows by discriminated `__type`. Schema fragments generate once and are imported by every query's generated module; parameterized fragments generate functions of their parameters where the target language supports it and monomorphic instantiations where it does not. Interface edges generate sum types over the declared alternatives plus a ref-only case for unlisted implementors, which makes the Section 4.5 placeholder path a compiler-enforced branch rather than a forgotten one.

### 20.4 Development mode

Origins SHOULD offer a development mode in which one-shot queries return, alongside data, the diagnostics a production origin withholds: full compile warnings, the explain payload inline, and slice annotations on every entity record. The exploratory REPL-against-an-API workflow that registration-based persisted queries destroyed is thereby retained; what production mode removes is only the unmemoized execution path's default-ness, not the workflow.

---

## 21. Remaining Open Questions

1. **Discriminant granularity in practice.** The design makes granularity a schema decision with agreement by construction, but which granularities keep both purge amplification and key cardinality acceptable is a per-domain empirical question. `lattice.purge.fanout` (Section 19.3) is the signal.
