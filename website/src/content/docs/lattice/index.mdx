...
## Why reads stay consistent (and exactly how much)

Splitting a query into independently-cached slices raises an obvious worry: if a
page is assembled from several requests that each hit the cache at a different
age, could it show a combination of facts that never coexisted? That is real
read skew, and Lattice's answer is neither "pin everything to one snapshot"
(which destroys the cache) nor "require identical snapshots" (which refetches
constantly on a busy database).

Instead, every response is a **validity window, not an instant**: alongside its
`Lattice-Snapshot` token it carries a `Lattice-Snapshot-Floor` marking the last
time anything it depends on changed. A page assembled from several reads is a
**consistent cut** exactly when their windows overlap (`max(floors) ≤
min(tokens)`), which makes the check a single cheap comparison. Unrelated writes
move tokens but not floors, so quiet pages pass for free; a relevant write is
repaired with an ordinary `no-cache` revalidation that usually comes back as a
`304`. The guarantee is honest: a passing assembly is exactly as consistent as
the same page fetched in one response, no more. When you want that guarantee
handed to you with zero client logic, a one-shot `slice=page` composes every
slice under a single snapshot, trading cacheability for structural consistency.

This is the part of the protocol whose semantics are easiest to get subtly
wrong, so it has its own walkthrough: **[Read consistency](./consistency/)**.
(Spec: [§13 Consistency Model](./spec/#_13-consistency-model), [§10.2
floors](./spec/#_10-caching).)
...
- [Read consistency](./consistency/): a plain-language tour of what a read
  guarantees, how independently-cached slices assemble into a consistent view,
  and exactly how much consistency you can rely on.
...
| Documentation strings + schema directives (declare/apply, locations, `repeatable`, reserved names) | §3.9 | Haskell (parse, validate, canonical, fusion); TS explorer surfaces docs + directives in hover/completion |
...
| Requested projections: per-type loader field sets, `explain` `projections` key, gateway-narrowed upstream selections | §3.1, §20.2 | Haskell (`planProjections`; the memory backend filters rows strictly, SQL backends map to `SELECT` lists) |
...
| Validity floors (`Lattice-Snapshot-Floor`): the per-key invalidation index, prefix-completeness gate, floors on data slices / point fetches / 304s | §10.2 | Haskell origin; TS ships the interval vocabulary (`validityIntervals`, `intervalsConsistent`) |
| Cross-slice consistent cuts: interval-overlap convergence, K-bounded `no-cache` repair, `qrConsistent` | §13.2 g2–g3 | Haskell client; model-checked in `wireform-lattice/tla/` |
| Composed page forms: one-shot `slice=page` single-snapshot responses, `503 lattice:snapshot-contention` | §6.5, §13.2 g7 | Haskell origin + client (`queryPage`) |
...
| Page subscriptions: `slice=page` multiplexed streams, single-snapshot bursts, per-`(slice, entity)` delta suppression | §12 | Haskell origin + client (`SubscribePage`) |
...
A few places cut a corner on purpose. Each is called out in the module's
haddock, so nothing here is missing by accident:

| What | Note |
|---|---|
| Declared cache policy (`cache` clause, Sections 3.4 and 10.9) | not implemented: the reference origin emits per-slice `Cache-Control` defaults, but does not parse, model, or apply schema-declared cache overrides |
...
| Gateway `nodes` batching | per (upstream, type, round); folding types into one query per (upstream, round) needs a round-level executor seam |
| Gateway v1 punts | parameterized computed fields elide across the seam; federated live SSE streams pass through unsplit (no `src`); interface-targeted federated edges and roots unsupported |
| `src` typing | typed on entity records; tombstone/elided/unchanged/error carry it in wire JSON only |
