...
None of this requires edge compute or custom CDN logic. A conforming deployment
runs behind unmodified Varnish (with the xkey vmod) or Fastly VCL. Surrogate-key
purging uses the standard `Surrogate-Key` header that both support natively. If
you are already serving HTTP traffic through a CDN, Lattice fits the shape you
already have.
