...
1. `Columnar.Bit` — **done**.
2. `VS.Vector` migration for primitive numeric / temporal / interval constructors (unit 2). **Done**, including `Parquet.Write.ColumnData` (the bridge no longer pays a `VS.convert` per column write — the values flow straight through).
3. `Utf8View` / `BinaryView` / `LargeUtf8View` / `LargeBinaryView` for the non-nullable variable-length constructors (unit 3) — **done**. `ColUtf8` / `ColBinary` / `ColLargeUtf8` / `ColLargeBinary` now hold the view types directly; the Arrow IPC reader hands back a view that wraps the source bytestring's `ForeignPtr` (zero copies, no per-row allocation), and the writer dumps the offsets + data buffers straight into the body via `vsByteString` (no-copy reinterpretation of `VS.Vector` as `ByteString`).
4. `NullableView` and `Nullable*View` for every nullable constructor (unit 4) — **done**. Primitive / temporal `Col*Maybe` carry a `NullableView a`. Variable-length `Col*Maybe` carry the matching `Nullable*View` (validity bitmap + dense view). `ColBoolMaybe` carries `NullableBoolView` (two bit vectors — wire format directly). `ColFixedSizeBinaryMaybe` carries `NullableFixedSizeBinaryView`.
5. Read-path zero-copy adoption (unit 5) — **done** for every host-LE primitive and variable-length path in `Arrow.Column`. The reader's `viewPrimVecLE` adopts the source `ByteString`'s `ForeignPtr` directly. Big-endian still byte-swap-copies.
6. `View` typeclass (unit 6) — **done**. `Arrow.View.View` covers `VS.Vector`, `VU.Vector Bit`, `V.Vector`, plus all four view types + their nullable variants.
7. Write path (unit 7) — **done**. The Arrow writer dumps view buffers directly via `vsByteString` (one ForeignPtr handoff per buffer). The new `writeNullableVar` helper does the same for nullable variable-length: three handoffs (validity bitmap, offsets, data) + one bit popcount.
8. Backward-compat helpers (unit 8) — **done**. `Arrow.View` exports `*FromVector` / `*ToVector` constructors and materialisers for every view type and nullable view; `Arrow.Column` exports the matching `nv*` family for primitive `NullableView`.
9. Tests (unit 9) — **done**. All eight existing test suites pass. `Arrow.View` round-trip tests added; nullable view round-trips added to the Arrow test suite.
10. Docs (unit 10) — **done**.

The migration is complete. Remaining performance work is in the separate "what's not at C/Rust speed" axis (slow per-`Text` UTF-8 validation on non-ASCII pages, the boxed `V.Vector ColumnArray` spine, the `RecordBatchDef` boxed-vector allocations) — all of those are independent of the views layer and tracked under their own follow-ups.
