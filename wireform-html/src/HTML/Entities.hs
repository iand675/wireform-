-- | The full HTML5 named-character-reference table.
--
-- This is the named character reference table from the WHATWG HTML
-- standard (<https://html.spec.whatwg.org/entities.json>). Both the
-- with-semicolon and (legacy) without-semicolon forms are folded into a
-- single map keyed by the bare name (without the leading @&@). The
-- @legacyEntities@ list enumerates names that may appear without a
-- terminating semicolon in legacy contexts (per the HTML standard's
-- error-recovery rules).
--
-- The table is generated from @entities.json@ as published by WHATWG.
-- Do not hand-edit; regenerate by running @scripts/regen-html-entities.py@.
